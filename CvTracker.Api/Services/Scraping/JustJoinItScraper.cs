...
/// <summary>
/// Scrapes job offers from JustJoin.it by fetching the public HTML page and
/// extracting the embedded <c>&lt;script type="application/ld+json"&gt;</c>
/// (schema.org <c>JobPosting</c>) block — no authentication required.
/// </summary>
public sealed class JustJoinItScraper : IScraper
{
    private readonly IHttpClientFactory _httpClientFactory;
    private readonly ILogger<JustJoinItScraper> _logger;

    /// <summary>Initialises the scraper with a named <see cref="IHttpClientFactory"/>.</summary>
    public JustJoinItScraper(IHttpClientFactory httpClientFactory, ILogger<JustJoinItScraper> logger)
    {
    ...
    public async Task<ScrapeResultDto> ScrapeAsync(Uri uri)
    {
        ...
        {
            _logger.LogWarning(ex, "JustJoin.it HTML fetch failed for {Uri}", uri);
        ...
        try
        {
            ...
            if (jsonLdRaw is null)
            {
                ...
                return Fail("No JobPosting JSON-LD found on JustJoin.it page.");
            }
            ...
            contractType ??= ExtractContractType(root);
        ...
        {
            _logger.LogWarning(ex, "JustJoin.it JSON-LD parsing failed for {Uri}", uri);
            return Fail(ex.Message);
        }
    }
    ...
    /// <summary>
    /// Maps schema.org <c>employmentType</c> values to our <see cref="ContractType"/> enum.
    /// Handles both a single string and an array of strings (takes the first recognisable value).
    /// </summary>
    private static ContractType? ExtractContractType(JsonElement root)
    {
        if (!root.TryGetProperty("employmentType", out var empType))
            return null;

        var raw = empType.ValueKind switch
        {
            JsonValueKind.String => empType.GetString(),
            JsonValueKind.Array  => empType.EnumerateArray()
